...
def main(args):
    ...
    try:
        modelname = os.path.join(args.param_root, '93.pt')
        yynet_dict = torch.load(modelname)
        print('loaded', modelname)
    except:
        yynet_dict = torch.load(os.path.join(args.param_root, '93.pt'))

    model.to(device)
    model.yynet.load_state_dict(yynet_dict)
    model.eval()
    model.set_mode('test')
